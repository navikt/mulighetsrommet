...
class FlywayDatabaseTestListener(private val config: DatabaseConfig) : BeforeSpecListener, AfterSpecListener {
    ...
    inline fun <T> run(block: TransactionalSession.() -> T): T = with(db.session) {
        try {
            connection.begin()
            transactional = true
            val tx = TransactionalSession(connection, returnGeneratedKeys, autoGeneratedKeys, strict)
            val result = block.invoke(tx)
            connection.commit()
            return result
        } catch (e: Exception) {
            connection.rollback()
    ...
    inline fun <T> runAndRollback(block: TransactionalSession.() -> T): T = with(db.session) {
        try {
            connection.begin()
            transactional = true
            val tx = TransactionalSession(connection, returnGeneratedKeys, autoGeneratedKeys, strict)
