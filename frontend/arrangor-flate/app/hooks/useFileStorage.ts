...
export function useFileStorage(dbName: string = "arrfiles"): FileStorage {
  ...
  async function store(
    ...
    try {
      ...
      const message = `Failed to store ${filesToStore.length} files in ${dbName}`;
  ...
  async function getAll(): Promise<StoredFile[]> {
    try {
      ...
      const message = "Failed to get all files from stoarge";
      // eslint-disable-next-line no-console
      console.error(message, e);
      return Promise.reject(message);
    }
  ...
  async function clear(): Promise<void> {
    try {
      ...
      const message = "Failed to clear filestorage";
      // eslint-disable-next-line no-console
      console.error(message, e);
      return Promise.reject(message);
    }
