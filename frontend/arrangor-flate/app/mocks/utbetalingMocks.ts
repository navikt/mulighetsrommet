import {
  ArrFlateUtbetaling,
  ArrFlateUtbetalingStatus,
  DelutbetalingStatus,
  NavEnhetStatus,
  NavEnhetType,
  TilsagnStatus,
  TilsagnType,
  UtbetalingType,
} from "api-client";

export const arrFlateUtbetaling: ArrFlateUtbetaling[] = [
  {
    id: "e48f9b35-855f-43aa-8b4d-a669013df34b",
    status: ArrFlateUtbetalingStatus.UTBETALT,
    godkjentAvArrangorTidspunkt: "2025-05-15T11:03:21.959059",
    createdAt: "2025-03-17T12:27:55.465944",
    tiltakstype: {
      navn: "Arbeidsforberedende trening",
    },
    gjennomforing: { id: "ded95e13-c121-45b1-a6b7-beadd85e2aa1", navn: "AFT Foobar" },
    arrangor: {
      id: "cc04c391-d733-4762-8208-b0dd4387a126",
      organisasjonsnummer: "973674471",
      navn: "BARNEVERNS- OG HELSENEMNDA I BUSKERUD OG OMEGN",
      slettet: false,
    },
    beregning: {
      type: "PRIS_PER_MANEDSVERK",
      belop: 10149,
      digest: "b3602d2a",
      deltakelser: [
        {
          id: "6248f41f-4029-4bce-baba-dc23cfd5a242",
          startDato: "2025-01-17",
          forstePeriodeStartDato: "2025-01-17",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 100.0,
          perioder: [
            { periode: { start: "2025-01-17", slutt: "2025-02-01" }, deltakelsesprosent: 100.0 },
          ],
          manedsverk: 0.48387,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
      ],
      stengt: [],
      antallManedsverk: 0.48,
    },
    betalingsinformasjon: { kontonummer: "10002427740", kid: null },
    periode: { start: "2025-01-01", slutt: "2025-02-01" },
    type: undefined,
    linjer: [
      {
        id: "59b56a35-4406-4c29-b5e6-535a78761044",
        tilsagn: {
          id: "ad77762c-eebb-4623-be6d-0c64da79f2dd",
          type: TilsagnType.TILSAGN,
          periode: { start: "2025-01-01", slutt: "2025-07-01" },
          belopBrukt: 51205,
          belopGjenstaende: 5234495,
          kostnadssted: {
            navn: "Nav tiltak Oslo",
            enhetsnummer: "0387",
            status: NavEnhetStatus.AKTIV,
            type: NavEnhetType.TILTAK,
            overordnetEnhet: "0300",
          },
          beregning: {
            type: "PRIS_PER_MANEDSVERK",
            input: {
              type: "PRIS_PER_MANEDSVERK",
              periode: { start: "2025-01-01", slutt: "2025-07-01" },
              sats: 20975,
              antallPlasser: 42,
            },
            output: { type: "PRIS_PER_MANEDSVERK", belop: 5285700 },
          },
          bestillingsnummer: "A-2025/11073-1",
          status: TilsagnStatus.GODKJENT,
        },
        status: DelutbetalingStatus.UTBETALT,
        statusSistOppdatert: "2025-05-15T11:03:22.772767",
        belop: 10149,
      },
    ],
  },
  {
    id: "db6f1689-6446-4f1b-900d-b06c469851f8",
    status: ArrFlateUtbetalingStatus.BEHANDLES_AV_NAV,
    godkjentAvArrangorTidspunkt: "2025-05-02T14:51:13.772419",
    createdAt: "2025-03-17T12:27:55.465944",
    tiltakstype: {
      navn: "Varig tilrettelagt arbeid i skjermet virksomhet",
    },
    gjennomforing: { id: "e6dfb756-3499-4d15-bd42-f25e01411443", navn: "November rain - VTA " },
    arrangor: {
      id: "cc04c391-d733-4762-8208-b0dd4387a126",
      organisasjonsnummer: "973674471",
      navn: "BARNEVERNS- OG HELSENEMNDA I BUSKERUD OG OMEGN",
      slettet: false,
    },
    beregning: {
      type: "PRIS_PER_MANEDSVERK",
      belop: 75816,
      digest: "3b1840bd",
      deltakelser: [
        {
          id: "519da1c4-b1d4-45b0-a20e-4520f365a036",
          startDato: "2024-12-03",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 50.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 50.0 },
          ],
          manedsverk: 1.0,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "55bd1c7f-dd0b-476c-a92d-9c5517c1d568",
          startDato: "2024-12-01",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 100.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 100.0 },
          ],
          manedsverk: 1.0,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "6f2dcd69-244c-4d1d-bfed-b99f3f4559d4",
          startDato: "2024-12-03",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 100.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 100.0 },
          ],
          manedsverk: 1.0,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "a34510af-0b4f-445c-8fe1-bcd83577eb88",
          startDato: "2024-11-05",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 49.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 49.0 },
          ],
          manedsverk: 0.5,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "b5419a6c-134c-4063-911d-0bb07f3c54ae",
          startDato: "2024-11-01",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 100.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 100.0 },
          ],
          manedsverk: 1.0,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
      ],
      stengt: [],
      antallManedsverk: 4.5,
    },
    betalingsinformasjon: { kontonummer: "63728787114", kid: null },
    periode: { start: "2025-01-01", slutt: "2025-02-01" },
    type: undefined,
    linjer: [
      {
        id: "8553457d-9faa-4ba8-b8aa-d32d7ec2db03",
        tilsagn: {
          id: "19c6ba1a-c7a0-4b3a-b4ba-a3aa94527d49",
          type: TilsagnType.TILSAGN,
          periode: { start: "2025-01-01", slutt: "2025-11-11" },
          belopBrukt: 0,
          belopGjenstaende: 15668640,
          kostnadssted: {
            navn: "Nav tiltak Oslo",
            enhetsnummer: "0387",
            status: NavEnhetStatus.AKTIV,
            type: NavEnhetType.TILTAK,
            overordnetEnhet: "0300",
          },
          beregning: {
            type: "PRIS_PER_MANEDSVERK",
            input: {
              type: "PRIS_PER_MANEDSVERK",
              periode: { start: "2025-01-01", slutt: "2025-11-11" },
              sats: 16848,
              antallPlasser: 90,
            },
            output: {
              type: "PRIS_PER_MANEDSVERK",
              belop: 15668640,
            },
          },
          bestillingsnummer: "A-2024/10819-3",
          status: TilsagnStatus.GODKJENT,
        },
        status: DelutbetalingStatus.OVERFORT_TIL_UTBETALING,
        statusSistOppdatert: "2025-05-21T00:00",
        belop: 15000,
      },
      {
        id: "f5d0898a-211f-4ad5-9489-ed847bf33dc8",
        tilsagn: {
          id: "06a3c371-1af8-488f-b111-9491783372c8",
          type: TilsagnType.EKSTRATILSAGN,
          periode: { start: "2025-01-01", slutt: "2025-06-01" },
          belopBrukt: 0,
          belopGjenstaende: 252720,
          kostnadssted: {
            navn: "Nav Arendal",
            enhetsnummer: "0906",
            status: NavEnhetStatus.AKTIV,
            type: NavEnhetType.LOKAL,
            overordnetEnhet: "1000",
          },
          beregning: {
            type: "PRIS_PER_MANEDSVERK",
            input: {
              type: "PRIS_PER_MANEDSVERK",
              periode: { start: "2025-01-01", slutt: "2025-06-01" },
              sats: 16848,
              antallPlasser: 3,
            },
            output: {
              type: "PRIS_PER_MANEDSVERK",
              belop: 252720,
            },
          },
          bestillingsnummer: "A-2024/10819-2",
          status: TilsagnStatus.GODKJENT,
        },
        status: DelutbetalingStatus.OVERFORT_TIL_UTBETALING,
        statusSistOppdatert: "2025-05-21T00:00",
        belop: 10000,
      },
    ],
  },
  {
    id: "a5499e34-9fb4-49d1-a37d-11810f6df19b",
    status: ArrFlateUtbetalingStatus.VENTER_PA_ENDRING,
    godkjentAvArrangorTidspunkt: null,
    createdAt: "2025-06-17T13:58:12.405867",
    tiltakstype: {
      navn: "Arbeidsforberedende trening",
    },
    gjennomforing: { id: "b3e1cfbb-bfb5-4b4b-b8a4-af837631ed51", navn: "Solrikt AFT" },
    arrangor: {
      id: "cc04c391-d733-4762-8208-b0dd4387a126",
      organisasjonsnummer: "973674471",
      navn: "BARNEVERNS- OG HELSENEMNDA I BUSKERUD OG OMEGN",
      slettet: false,
    },
    beregning: {
      type: "PRIS_PER_MANEDSVERK",
      belop: 242904,
      digest: "db0c7c6e",
      deltakelser: [
        {
          id: "ff07c9c2-dff3-4e81-bd5a-40bb19108cc1",
          startDato: "2024-07-15",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 60.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 60.0 },
          ],
          manedsverk: 1.0,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "f7284cdb-9b8f-4431-9808-b1bcc9ae7494",
          startDato: "2024-07-16",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 100.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 100.0 },
          ],
          manedsverk: 1.0,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "32b05c15-e666-46ad-8fae-ee7a5263fa81",
          startDato: "2024-12-19",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 100.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 100.0 },
          ],
          manedsverk: 1.0,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "a06a5562-78ac-4564-992c-cae22d9707f2",
          startDato: "2024-12-19",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 100.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 100.0 },
          ],
          manedsverk: 1.0,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "44af1d55-33f6-4198-9ebd-3f09e50e9b84",
          startDato: "2024-08-29",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 100.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 100.0 },
          ],
          manedsverk: 1.0,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "b78c320c-e67e-4326-ab2a-441ac5c8c63a",
          startDato: "2024-07-08",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-08",
          sistePeriodeDeltakelsesprosent: 60.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-01-09" }, deltakelsesprosent: 60.0 },
          ],
          manedsverk: 0.25806,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "7efeb453-805b-4fda-a413-e1723ff93a1b",
          startDato: "2024-10-14",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-14",
          sistePeriodeDeltakelsesprosent: 80.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-01-15" }, deltakelsesprosent: 80.0 },
          ],
          manedsverk: 0.45161,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "ec1c4596-cbd9-4fb0-8aa7-0c7b41ced5de",
          startDato: "2024-11-08",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 100.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 100.0 },
          ],
          manedsverk: 1.0,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "5c74c72c-52b0-49d4-8cf2-8466656f735a",
          startDato: "2024-09-23",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 80.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 80.0 },
          ],
          manedsverk: 1.0,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "9ba5eb6e-c230-4f72-8114-9f24d7652485",
          startDato: "2024-10-01",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-07",
          sistePeriodeDeltakelsesprosent: 60.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-01-08" }, deltakelsesprosent: 60.0 },
          ],
          manedsverk: 0.22581,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "4616dda7-e9ff-474e-8470-982a675b2ca5",
          startDato: "2024-11-06",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 100.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 100.0 },
          ],
          manedsverk: 1.0,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "bca5c636-bcc5-433f-96e8-e6f6230a11c1",
          startDato: "2025-01-27",
          forstePeriodeStartDato: "2025-01-27",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 100.0,
          perioder: [
            { periode: { start: "2025-01-27", slutt: "2025-02-01" }, deltakelsesprosent: 100.0 },
          ],
          manedsverk: 0.16129,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "9f72fa31-d7e4-4e97-ab6c-0f33e86fe22b",
          startDato: "2024-10-21",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 100.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 100.0 },
          ],
          manedsverk: 1.0,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "f40a502c-c983-4e89-b446-3679113dc50a",
          startDato: "2024-08-19",
          forstePeriodeStartDato: "2025-01-01",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 77.0,
          perioder: [
            { periode: { start: "2025-01-01", slutt: "2025-02-01" }, deltakelsesprosent: 77.0 },
          ],
          manedsverk: 1.0,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
        {
          id: "b08e99ff-4406-4ceb-9299-b6e9f1afaa30",
          startDato: "2025-01-17",
          forstePeriodeStartDato: "2025-01-17",
          sistePeriodeSluttDato: "2025-01-31",
          sistePeriodeDeltakelsesprosent: 100.0,
          perioder: [
            { periode: { start: "2025-01-17", slutt: "2025-02-01" }, deltakelsesprosent: 100.0 },
          ],
          manedsverk: 0.48387,
          person: { navn: "Nordmann, Ola", fodselsdato: "1993-11-01", fodselsaar: 1993 },
        },
      ],
      stengt: [],
      antallManedsverk: 11.58,
    },
    betalingsinformasjon: { kontonummer: "10002427740", kid: null },
    periode: { start: "2025-01-01", slutt: "2025-02-01" },
    type: undefined,
    linjer: [],
  },
  {
    id: "585a2834-338a-4ac7-82e0-e1b08bfe1408",
    status: ArrFlateUtbetalingStatus.BEHANDLES_AV_NAV,
    godkjentAvArrangorTidspunkt: "2025-06-05T09:36:11.510229",
    createdAt: "2025-06-05T09:36:11.510957",
    tiltakstype: {
      navn: "Arbeidsforberedende trening",
    },
    gjennomforing: {
      id: "d29cb67c-8e68-4ece-90dc-ff21c498aa3f",
      navn: "AFT - Arbeidsforberedende trening - Team tiltakspenger",
    },
    arrangor: {
      id: "cc04c391-d733-4762-8208-b0dd4387a126",
      organisasjonsnummer: "973674471",
      navn: "BARNEVERNS- OG HELSENEMNDA I BUSKERUD OG OMEGN",
      slettet: false,
    },
    beregning: { type: "FRI", belop: 234, digest: "000001d4" },
    betalingsinformasjon: { kontonummer: "10002427740", kid: null },
    periode: { start: "2025-05-01", slutt: "2025-08-02" },
    type: UtbetalingType.INVESTERING,
    linjer: [],
  },
  {
    id: "153bc6f0-0c5f-4555-9447-b88ea0cc60f2",
    status: ArrFlateUtbetalingStatus.OVERFORT_TIL_UTBETALING,
    godkjentAvArrangorTidspunkt: null,
    createdAt: "2025-04-01T12:03:49.756508",
    tiltakstype: {
      navn: "Arbeidsforberedende trening",
    },
    gjennomforing: {
      id: "6d71a9c5-c920-4d56-bc3b-2da07e4b6100",
      navn: "AFT - Arbeidsforberedende trening - Team tiltakspenger",
    },
    arrangor: {
      id: "cc04c391-d733-4762-8208-b0dd4387a126",
      organisasjonsnummer: "973674471",
      navn: "BARNEVERNS- OG HELSENEMNDA I BUSKERUD OG OMEGN",
      slettet: false,
    },
    beregning: { type: "FRI", belop: 500, digest: "000003e8" },
    betalingsinformasjon: { kontonummer: "63728787114", kid: null },
    periode: { start: "2025-01-01", slutt: "2025-02-01" },
    type: UtbetalingType.KORRIGERING,
    linjer: [
      {
        id: "b1a3727a-8bf7-4470-a9b6-71803c99846e",
        tilsagn: {
          id: "08c6234e-1fc2-432b-b3e2-a0d30f1b2cf7",
          type: TilsagnType.TILSAGN,
          periode: { start: "2025-01-01", slutt: "2025-07-01" },
          belopBrukt: 500,
          belopGjenstaende: 1509700,
          kostnadssted: {
            navn: "Nav Hallingdal",
            enhetsnummer: "0617",
            status: NavEnhetStatus.AKTIV,
            type: NavEnhetType.LOKAL,
            overordnetEnhet: "0600",
          },
          beregning: {
            type: "PRIS_PER_MANEDSVERK",
            input: {
              type: "PRIS_PER_MANEDSVERK",
              periode: { start: "2025-01-01", slutt: "2025-07-01" },
              sats: 20975,
              antallPlasser: 12,
            },
            output: {
              type: "PRIS_PER_MANEDSVERK",
              belop: 1510200,
            },
          },
          bestillingsnummer: "A-2025/11198-3",
          status: TilsagnStatus.GODKJENT,
        },
        status: DelutbetalingStatus.OVERFORT_TIL_UTBETALING,
        statusSistOppdatert: undefined,
        belop: 500,
      },
    ],
  },
];
