...
export function TilsagnDetaljer() {
  ...
  function kanBeslutte(opprettetAv: string): boolean {
    return (
      ansatt?.navIdent !== opprettetAv && ansatt?.roller.includes(NavAnsattRolle.OKONOMI_BESLUTTER)
    );
  }

  const visHandlingerMeny =
    tilsagn.status === TilsagnStatus.RETURNERT || tilsagn.status === TilsagnStatus.GODKJENT;
    ...
    <main>
      ...
      <ContentBox>
        <Box background="bg-default" padding={"5"}>
          ...
          {tilsagn.status === TilsagnStatus.RETURNERT && (
            <AvvistAlert godkjennelse={tilsagn.godkjennelse} />
          )}
          {tilsagn.status === TilsagnStatus.TIL_ANNULLERING && tilsagn.annullering && (
          ...
          >
            ...
            <div>
              ...
              <HStack gap="2" justify={"end"}>
                {kanBeslutte(tilsagn.godkjennelse.opprett.navIdent) &&
                ...
                {kanBeslutte(tilsagn.annullering!.opprett.navIdent) &&
