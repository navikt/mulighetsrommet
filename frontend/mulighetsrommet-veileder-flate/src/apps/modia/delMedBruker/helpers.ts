...
import { Brukerdata, VeilederflateTiltak } from "@arbeidsmarkedstiltak/api-client";
...
export function utledDelMedBrukerTekst(tiltak: VeilederflateTiltak, veiledernavn?: string) {
  ...
  const templatedDeletekst = deletekst.replaceAll("<tiltaksnavn>", tiltak.tiltakstype.navn);
...
export function getDelMedBrukerTekst(tiltak: VeilederflateTiltak): string | null {
  return tiltak.faneinnhold?.delMedBruker ?? tiltak.tiltakstype.faneinnhold?.delMedBruker ?? null;
}
