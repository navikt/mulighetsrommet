...
export function useBrukerHarRettPaaTiltak() {
  ...
  const innsatsgruppeForGjennomforing = data?.tiltakstype?.innsatsgruppe?.nokkel ?? Innsatsgruppe.STANDARD_INNSATS;
  const brukersInnsatsgruppe = brukerdata?.innsatsgruppe;

  const godkjenteInnsatsgrupper = brukersInnsatsgruppe ? utledInnsatsgrupperFraInnsatsgruppe(brukersInnsatsgruppe) : [];
