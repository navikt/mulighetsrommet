...
- **Hvofor feiler `terraform apply` første gang det kjøres?** Første gang `terraform apply` kjøres så kan det være at
  ...
  typisk ta litt tid før disse tabellene blir opprettet av datastreametn. Det burde fungere å kjøre `terraform apply` på
...
- **Hvorfor ser jeg ikke oppdatert data i BigQuery?** Standard innstilling for `bigquery_table_freshness` er satt til
  1t, som betyr at det kan ta inntil en time før endringer i data er tilgjengelig i BigQuery. Dette kan endres i
