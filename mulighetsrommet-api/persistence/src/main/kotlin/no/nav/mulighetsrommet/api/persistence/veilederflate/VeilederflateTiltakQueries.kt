...
import no.nav.mulighetsrommet.api.veilederflate.EstimertVentetid
import no.nav.mulighetsrommet.api.veilederflate.VeilederflateArrangor
import no.nav.mulighetsrommet.api.veilederflate.VeilederflateArrangorKontaktperson
...
class VeilederflateTiltakQueries(private val session: Session) : VeilederflateTiltakQueryHandler {

    override fun get(id: UUID): VeilederflateTiltakDbo? {
        @Language("PostgreSQL")
        val query = """
            select *
    ...
    ): List<VeilederflateTiltakDbo> = with(session) {
...
private fun Row.toTiltaksgjennomforing(): VeilederflateTiltakDbo {
    ...
    val stengt = stringOrNull("stengt_perioder_json")
        ?.let { Json.decodeFromString<List<VeilederflateTiltakDbo.StengtPeriode>>(it) }
        ?: emptyList()

    val status = GjennomforingStatusType.valueOf(string("status"))

    return VeilederflateTiltakDbo(
        id = uuid("id"),
