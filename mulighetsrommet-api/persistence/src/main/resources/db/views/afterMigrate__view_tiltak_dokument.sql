...
from tiltak_dokument ig
         ...
         left join lateral (select jsonb_agg(
                            ...
                            where adm.tiltak_dokument_id = ig.id) on true
         left join lateral (select jsonb_agg(
                                           jsonb_build_object(
                            ...
                            from tiltak_dokument_nav_enhet ig_enhet
                                     join nav_enhet enhet on enhet.enhetsnummer = ig_enhet.enhetsnummer
                            where ig_enhet.tiltak_dokument_id = ig.id) on true
         left join lateral (select jsonb_agg(
                                           jsonb_build_object(
                                                   'navIdent', ansatt.nav_ident,
                                                   'navn', concat(ansatt.fornavn, ' ', ansatt.etternavn),
                            ...
                            where kp.tiltak_dokument_id = ig.id) on true
         left join lateral (select jsonb_agg(
                                           jsonb_build_object(
                                                   'id', akp.id,
                                                   'navn', akp.navn,
                            ...
                            from tiltak_dokument_arrangor_kontaktperson ig_akp
                                     join arrangor_kontaktperson akp on akp.id = ig_akp.arrangor_kontaktperson_id
                            where ig_akp.tiltak_dokument_id = ig.id) on true
