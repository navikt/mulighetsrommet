...
class ArrangorflateTilsagnQueriesTest : FunSpec({
    ...
    val tilsagnAft = TilsagnFixtures.createTilsagn(aft1.id, lopenummer = 1, type = TilsagnType.TILSAGN)
    val ekstratilsagnAft = TilsagnFixtures.createTilsagn(aft1.id, lopenummer = 2, type = TilsagnType.EKSTRATILSAGN)
    val investeringtilsagnAft = TilsagnFixtures.createTilsagn(aft1.id, lopenummer = 3, type = TilsagnType.INVESTERING)
    val tilsagnIkkeGodkjent = TilsagnFixtures.createTilsagn(aft1.id, lopenummer = 6, type = TilsagnType.TILSAGN)
    val tilsagnVta = TilsagnFixtures.createTilsagn(vta1.id, lopenummer = 4, type = TilsagnType.TILSAGN)
    val tilsagnAnnenArrangor = TilsagnFixtures.createTilsagn(aftHosAnnenArrangor.id, lopenummer = 5, type = TilsagnType.TILSAGN)
