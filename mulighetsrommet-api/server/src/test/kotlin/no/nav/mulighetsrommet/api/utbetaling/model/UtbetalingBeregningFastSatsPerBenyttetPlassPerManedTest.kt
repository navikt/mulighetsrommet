...
class UtbetalingBeregningFastSatsPerBenyttetPlassPerManedTest : FunSpec({
    val sats = 100.NOK
    ...
    context("beregning for Fast sats per benyttet tiltaksplass per måned") {
        test("beløp beregnes fra månedsverk til deltakere og sats") {
            ...
            result.input shouldBe UtbetalingBeregningFastSatsPerBenyttetPlassPerManed.Input(
            ...
            result.output shouldBe UtbetalingBeregningFastSatsPerBenyttetPlassPerManed.Output(
                pris = 300.NOK,
                deltakelser = setOf(
        ...
        test("beregnet beløp rundes av til nærmeste hele krone") {
            ...
            val gjennomforing =
                createGjennomforingForForhandsgodkjentPris(periode = periode, sats = 20205.NOK)
            val deltakere = listOf(
                createDeltaker(
                    periode = deltakelse1,
                    deltakelsesmengder = listOf(deltakelsesmengde(periode.start, 100.0)),
            ...
            result.output shouldBe UtbetalingBeregningFastSatsPerBenyttetPlassPerManed.Output(
                ...
                deltakelser = setOf(
                    UtbetalingBeregningOutputDeltakelse(
                        deltakere[0].id,
                        setOf(UtbetalingBeregningOutputDeltakelse.BeregnetPeriode(deltakelse1, 0.21739, 20205.NOK)),
                    ),
                    UtbetalingBeregningOutputDeltakelse(
                        deltakere[1].id,
                        setOf(UtbetalingBeregningOutputDeltakelse.BeregnetPeriode(deltakelse2, 0.26087, 20205.NOK)),
                    ),
                ),
            )
        }
        ...
        test("utbetaling og tilsagn er likt for forskjellige perioder av en måned") {
            ...
            (2..31).forEach { dayOfMonth ->
                ...
                val utbetaling = FastSatsPerTiltaksplassPerManedBeregning.beregn(gjennomforing, periode, deltakere)

                val tilsagn = TilsagnBeregningAvtaltPrisPerBenyttetPlassPerManed.beregn(
        ...
        test("to deltakere over 2 måneder gir fire månedsverk") {
            ...
            result.output shouldBe UtbetalingBeregningFastSatsPerBenyttetPlassPerManed.Output(
                ...
                deltakelser = setOf(
                    UtbetalingBeregningOutputDeltakelse(
                        deltakere[0].id,
                        setOf(UtbetalingBeregningOutputDeltakelse.BeregnetPeriode(periode, 2.0, 10.NOK)),
                    ),
                    UtbetalingBeregningOutputDeltakelse(
                        deltakere[1].id,
                        setOf(UtbetalingBeregningOutputDeltakelse.BeregnetPeriode(periode, 2.0, 10.NOK)),
