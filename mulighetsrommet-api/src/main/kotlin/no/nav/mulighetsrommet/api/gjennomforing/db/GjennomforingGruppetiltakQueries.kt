...
class GjennomforingGruppetiltakQueries(private val session: Session) {
    fun upsert(gjennomforing: GjennomforingGruppeDbo) = withTransaction(session) {
        @Language("PostgreSQL")
        val query = """
            insert into gjennomforing (id, tiltakstype_id, arrangor_id)
            values (:gjennomforing_id::uuid, :tiltakstype_id::uuid, :arrangor_id::uuid)
        ...
        val params = mapOf(
            ...
            "sted_for_gjennomforing" to gjennomforing.stedForGjennomforing,
    ...
    fun get(id: UUID): Gjennomforing? {
        ...
        return session.single(queryOf(query, id)) { it.toGruppetiltak() }
    }
    ...
    fun getPrismodell(id: UUID): PrismodellType? {
        ...
        val query = """
            ...
            from gjennomforing_gruppetiltak
                join avtale on avtale_id = avtale.id
            where gjennomforing_id = ?::uuid
        """.trimIndent()
    ...
    fun getAll(
        ...
        return queryOf(query, parameters + pagination.parameters)
            .mapPaginated { it.toGruppetiltak() }
            .runWithSession(this)
    }
...
private fun Row.toGruppetiltak(): Gjennomforing {
