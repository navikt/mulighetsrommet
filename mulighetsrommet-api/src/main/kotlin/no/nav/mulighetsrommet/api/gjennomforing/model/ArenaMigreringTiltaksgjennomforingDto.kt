    ...
    companion object {
        fun from(
            ...
            val enhetsnummer = gjennomforing.arena?.ansvarligNavEnhet
                ?: (gjennomforing as? AvtaleGjennomforing)?.kontorstruktur?.firstOrNull()?.region?.enhetsnummer?.value
                ?: error("navRegion or arenaAnsvarligEnhet was null! Should not be possible!")

            val status = when (gjennomforing) {
                is AvtaleGjennomforing -> gjennomforing.status.type
                is EnkeltplassGjennomforing -> gjennomforing.status
                is ArenaGjennomforing -> gjennomforing.status
            }
            ...
            return ArenaMigreringTiltaksgjennomforingDto(
                ...
                apentForInnsok = when (gjennomforing) {
                    is AvtaleGjennomforing -> gjennomforing.apentForPamelding
                    is EnkeltplassGjennomforing, is ArenaGjennomforing -> false
                },
