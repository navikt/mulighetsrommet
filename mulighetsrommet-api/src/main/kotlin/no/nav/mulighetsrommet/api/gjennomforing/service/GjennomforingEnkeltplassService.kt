...
class GjennomforingEnkeltplassService(
    ...
    fun create(create: UpsertGjennomforingEnkeltplass, agent: Agent): Validated<GjennomforingEnkeltplass> = db.transaction {
        ...
        upsert(create)
            .also {
                if (agent is NavIdent || agent is Tiltaksadministrasjon) {
                    createTotrinnskontroll(it.id, Totrinnskontroll.Type.OPPRETT, agent)
                }
            }
    ...
    private fun QueryContext.publishTiltaksgjennomforingV2ToKafka(gjennomforing: GjennomforingEnkeltplass) {
        ...
        val dto = TiltaksgjennomforingV2Mapper.fromGjennomforingEnkeltplass(gjennomforing, totrinnskontroll)
