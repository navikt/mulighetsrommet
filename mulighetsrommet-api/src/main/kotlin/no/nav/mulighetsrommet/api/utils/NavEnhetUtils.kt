...
object NavEnhetUtils {
    fun isRelevantEnhetStatus(status: Norg2EnhetStatus): Boolean = status in listOf(
        Norg2EnhetStatus.UNDER_ETABLERING,
        Norg2EnhetStatus.UNDER_AVVIKLING,
        Norg2EnhetStatus.AKTIV,
    )

    fun isRelevantEnhetType(type: Norg2Type): Boolean = type in listOf(
        Norg2Type.FYLKE,
        Norg2Type.LOKAL,
    )

    fun toEnhetId(enhet: Norg2EnhetDto): String = "enhet.${enhet.type.name.lowercase()}.${enhet.enhetNr}"

    fun toType(type: String?): String = when (type) {
    ...
    fun toStatus(status: String?): String = when (status) {
