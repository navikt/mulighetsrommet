...
class OppgaverService(val db: ApiDatabase) {
    ...
    private fun TilsagnDto.createdAt(): LocalDateTime {
        return when (this.status) {
            TilsagnStatus.TIL_GODKJENNING -> this.godkjennelse.opprett.tidspunkt
            TilsagnStatus.RETURNERT -> {
                requireNotNull(this.godkjennelse.beslutt)
                this.godkjennelse.beslutt.tidspunkt
            }
            TilsagnStatus.TIL_ANNULLERING -> {
                requireNotNull(this.annullering)
                this.annullering.opprett.tidspunkt
            }
            else -> throw IllegalStateException("Ukjent tilsagnstatus")
        }
    }
