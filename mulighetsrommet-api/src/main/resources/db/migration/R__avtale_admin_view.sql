...
with personopplysninger as (select avtale_id,
                                   jsonb_agg(personopplysning) as personopplysninger_json
                            from avtale_personopplysning
                            group by avtale_id),
     administratorer as (select avtale_id,
                                jsonb_agg(
                                        jsonb_build_object(
                         ...
                         group by avtale_id),
     nav_enheter as (select avtale_id,
                            jsonb_agg(
                                    jsonb_build_object(
                     ...
                     from avtale_nav_enhet
                              join nav_enhet on nav_enhet.enhetsnummer = avtale_nav_enhet.enhetsnummer
                     group by avtale_id),
     opsjon_logg as (select avtale_id,
                            jsonb_agg(
                     ...
                     group by avtale_id),
     amo_kategorisering as (select avtale_id,
                                   jsonb_build_object(
                                           ...
                                           'sertifiseringer',
                                           coalesce((select jsonb_strip_nulls(jsonb_agg(jsonb_build_object(
                                                   'label', s.label,
     ...
     arrangor_underenheter as (select avtale_id,
                                      jsonb_agg(
                                              jsonb_build_object(
                                                      'id', arrangor.id,
                               ...
                               group by avtale_id),
     arrangor_kontaktpersoner as (select avtale_id,
                                         jsonb_agg(
                                                 jsonb_build_object(
...
from avtale
         ...
         left join personopplysninger on avtale.id = personopplysninger.avtale_id
         left join administratorer on avtale.id = administratorer.avtale_id
         left join nav_enheter on avtale.id = nav_enheter.avtale_id
         left join opsjon_logg on avtale.id = opsjon_logg.avtale_id
         left join amo_kategorisering on avtale.id = amo_kategorisering.avtale_id
         left join arrangor_underenheter on avtale.id = arrangor_underenheter.avtale_id
         left join arrangor_kontaktpersoner on avtale.id = arrangor_kontaktpersoner.avtale_id
