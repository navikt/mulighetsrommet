...
import no.nav.mulighetsrommet.api.avtale.mapper.AvtaleDboMapper.toAvtaleRequest
import no.nav.mulighetsrommet.api.avtale.model.*
import no.nav.mulighetsrommet.api.databaseConfig
import no.nav.mulighetsrommet.api.fixtures.*
import no.nav.mulighetsrommet.api.fixtures.AvtaleFixtures.avtaleDetaljerRequest
import no.nav.mulighetsrommet.api.fixtures.AvtaleFixtures.avtaleVeilederinfoRequest
...
class AvtaleValidatorTest : FunSpec({
    ...
    val avtaleRequest = AvtaleFixtures.avtaleRequest.copy(
    ...
    test("should accumulate errors when request has multiple issues") {
        ...
        val request = avtaleRequest.copy(
            detaljer = AvtaleFixtures.avtaleDetaljerRequest.copy(
            ...
            veilederinformasjon = AvtaleFixtures.avtaleVeilederinfoRequest.copy(
        ...
        validator.validateCreateAvtale(request).shouldBeLeft().shouldContainAll(
            listOf(
    ...
    test("Avtalenavn må være minst 5 tegn når avtalen er opprettet i Admin-flate") {
        ...
        val request = avtaleRequest.copy(detaljer = AvtaleFixtures.avtaleDetaljerRequest.copy(navn = "Avt"))
    ...
    test("skal validere at Nav-fylke og Nav-enheter er påkrevd") {
        ...
        val request = avtaleRequest.copy(
            veilederinformasjon = AvtaleFixtures.avtaleVeilederinfoRequest.copy(navEnheter = listOf()),
        )
    ...
    test("amoKategorisering er påkrevd hvis gruppe amo") {
        val validator = createValidator()
        validator.validateCreateAvtale(
            gruppeAmo.copy(detaljer = AvtaleFixtures.avtaleDetaljerRequest.copy(amoKategorisering = null)),
        ).shouldBeLeft(
    ...
    test("avtaletype må stemme overens med tiltakstypen") {
        ...
        forAll(
            row(
                forhaandsgodkjent.copy(detaljer = AvtaleFixtures.avtaleDetaljerRequest.copy(avtaletype = Avtaletype.RAMMEAVTALE)),
                FieldError(
                    "/avtaletype",
            ...
            row(
                avtaleTypeAvtale.copy(detaljer = AvtaleFixtures.avtaleDetaljerRequest.copy(avtaletype = Avtaletype.OFFENTLIG_OFFENTLIG)),
                FieldError("/avtaletype", "Offentlig-offentlig samarbeid er ikke tillatt for tiltakstype Oppfølging"),
            ),
            row(
                gruppeAmo.copy(detaljer = AvtaleFixtures.avtaleDetaljerRequest.copy(avtaletype = Avtaletype.FORHANDSGODKJENT)),
                FieldError(
                    "/avtaletype",
    ...
    context("når avtalen allerede eksisterer") {
        test("Skal ikke kunne endre opsjonsmodell eller avtaletype når opsjon er registrert") {
            ...
            database.run {
                queries.avtale.upsertDetaljer(
                    id = AvtaleFixtures.gruppeAmo.id,
                    AvtaleFixtures.avtaleDetaljerDbo.copy(
                        startDato = startDato,
                        sluttDato = startDato.plusYears(1),
                        opsjonsmodell = OpsjonsmodellDbo(
                            OpsjonsmodellType.TO_PLUSS_EN_PLUSS_EN,
                            startDato.plusYears(4),
                            null,
                        ),
                    ),
                )
            ...
            val avtale = gruppeAmo.copy(
                detaljer = avtaleDetaljerRequest.copy(
                    avtaletype = Avtaletype.AVTALE,
        ...
        context("når avtalen har gjennomføringer") {
            ...
            test("skal godta at gjennomføring har andre Nav-enheter enn avtalen") {
                ...
                val request = avtaleVeilederinfoRequest.copy(
                    navEnheter = listOf(NavEnhetFixtures.Oslo.enhetsnummer, NavEnhetFixtures.Sagene.enhetsnummer),
                )

                createValidator().validateNavEnheter(request.navEnheter).shouldBeRight()
            }
            ...
            test("kan ikke endre tiltakstype hvis prismodell er inkompatibel") {
                ...
                val request = avtaleDetaljerRequest.copy(
                ...
                createValidator().validateUpdateDetaljer(oppfolgingMedRammeAvtale.id, request, previous!!)
    ...
    context("status endringer") {
        test("status blir UTKAST når avtalen lagres uten en arrangør") {
            createValidator().validateCreateAvtale(avtaleRequest.copy(detaljer = avtaleDetaljerRequest.copy(arrangor = null)))
        ...
        test("status forblir AVBRUTT på en avtale som allerede er AVBRUTT") {
            ...
            val request = AvtaleFixtures.avtaleDetaljerRequest.copy(
                startDato = today,
                sluttDato = today,
            )
            val previous = database.run { queries.avtale.get(avtale.id) }

            createValidator().validateUpdateDetaljer(avtale.id, request, previous!!).shouldBeRight().should {
