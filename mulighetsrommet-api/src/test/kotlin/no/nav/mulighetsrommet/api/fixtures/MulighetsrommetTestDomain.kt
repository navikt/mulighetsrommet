...
data class MulighetsrommetTestDomain(
    ...
    val gjennomforinger: List<GjennomforingGruppeDbo> = listOf(),
    val enkeltplasser: List<GjennomforingDbo> = listOf(),
    ...
    fun setup(session: Session): MulighetsrommetTestDomain {
        ...
        with(context) {
            ...
            gjennomforinger.forEach { queries.gruppetiltak.upsert(it) }
            enkeltplasser.forEach { queries.gjennomforing.upsert(it) }
