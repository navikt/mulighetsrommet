    ...
    context("avtale") {
        test("lagre gjennomføring") {
            database.runAndRollback {
                ...
                queries.gjennomforing.getAvtaleGjennomforingOrError(Oppfolging1.id) should {
                    ...
                    it.administratorer shouldBe listOf(
                        AvtaleGjennomforing.Administrator(
                ...
                queries.gjennomforing.getAvtaleGjennomforing(Oppfolging1.id) shouldBe null
            }
        }
        ...
        test("Administratorer crud") {
            database.runAndRollback {
                ...
                queries.gjennomforing.upsertGjennomforingAvtale(gjennomforing)

                queries.gjennomforing.getAvtaleGjennomforing(gjennomforing.id)?.administratorer.shouldContainExactlyInAnyOrder(
                    AvtaleGjennomforing.Administrator(
        ...
        test("navEnheter crud") {
            database.runAndRollback { session ->
                ...
                queries.gjennomforing.getAvtaleGjennomforingOrError(Oppfolging1.id).kontorstruktur.shouldHaveSize(1)
                    .first()
                    .should {
                ...
                queries.gjennomforing.upsertGjennomforingAvtale(Oppfolging1.copy(navEnheter = setOf()))
                queries.gjennomforing.getAvtaleGjennomforingOrError(Oppfolging1.id).kontorstruktur.shouldBeEmpty()
            }
        }
        ...
        test("Nav kontaktperson") {
            database.runAndRollback {
                ...
                queries.gjennomforing.getAvtaleGjennomforing(Oppfolging1.id)?.kontaktpersoner shouldContainExactlyInAnyOrder listOf(
                ...
                queries.gjennomforing.getAvtaleGjennomforing(Oppfolging1.id)?.kontaktpersoner shouldBe listOf(
                    GjennomforingKontaktperson(
                        navIdent = NavIdent("DD1"),
                        navn = "Donald Duck",
                        mobilnummer = "12345678",
        ...
        test("arrangør kontaktperson") {
            ...
            database.runAndRollback { session ->
                ...
                queries.gjennomforing.upsertGjennomforingAvtale(Oppfolging1.copy(arrangorKontaktpersoner = listOf(thomas.id)))
                queries.gjennomforing.getAvtaleGjennomforingOrError(Oppfolging1.id).should {
                    it.arrangor.kontaktpersoner shouldContainExactly listOf(toGjennomforingArrangorKontaktperson(thomas))
                }

                queries.gjennomforing.upsertGjennomforingAvtale(Oppfolging1.copy(arrangorKontaktpersoner = emptyList()))
                queries.gjennomforing.getAvtaleGjennomforingOrError(Oppfolging1.id).should {
        ...
        test("Publisert må settes eksplisitt") {
            database.runAndRollback {
                ...
                queries.gjennomforing.getAvtaleGjennomforing(Oppfolging1.id)?.publisert shouldBe false

                queries.gjennomforing.setPublisert(Oppfolging1.id, true)
                queries.gjennomforing.getAvtaleGjennomforing(Oppfolging1.id)?.publisert shouldBe true
            }
        }
        ...
        test("skal sette åpent for påmelding") {
            database.runAndRollback {
                queries.gjennomforing.upsertGjennomforingAvtale(Oppfolging1)
                queries.gjennomforing.getAvtaleGjennomforingOrError(Oppfolging1.id).apentForPamelding shouldBe true

                queries.gjennomforing.setApentForPamelding(Oppfolging1.id, false)
                queries.gjennomforing.getAvtaleGjennomforingOrError(Oppfolging1.id).apentForPamelding shouldBe false
            }
        }
        ...
        test("oppdater status") {
            database.runAndRollback {
                ...
                queries.gjennomforing.getAvtaleGjennomforingOrError(id).status shouldBe GjennomforingStatus.Avbrutt(
                    tidspunkt = tidspunkt,
                ...
                queries.gjennomforing.getAvtaleGjennomforingOrError(id).status shouldBe GjennomforingStatus.Avlyst(
                    tidspunkt = tidspunkt,
                    aarsaker = listOf(AvbrytGjennomforingAarsak.FEILREGISTRERING),
                    forklaring = null,
                )
                ...
                queries.gjennomforing.getAvtaleGjennomforingOrError(id).status shouldBe GjennomforingStatus.Gjennomfores
            }
        }
        ...
        test("lagre faneinnhold") {
            ...
            database.runAndRollback {
                queries.gjennomforing.upsertGjennomforingAvtale(Oppfolging1.copy(faneinnhold = faneinnhold))
        ...
        test("lagre amoKategoriserng") {
            ...
            database.runAndRollback {
                queries.gjennomforing.upsertGjennomforingAvtale(Oppfolging1.copy(amoKategorisering = amo))
                queries.gjennomforing.getAvtaleGjennomforingOrError(Oppfolging1.id).should {
                    it.amoKategorisering shouldBe amo
                }

                queries.gjennomforing.upsertGjennomforingAvtale(Oppfolging1.copy(amoKategorisering = null))
                queries.gjennomforing.getAvtaleGjennomforingOrError(Oppfolging1.id).should {
        ...
        test("stengt hos arrangør lagres og hentes i periodens rekkefølge") {
            database.runAndRollback {
                ...
                queries.gjennomforing.getAvtaleGjennomforingOrError(Oppfolging1.id).stengt shouldContainExactly listOf(
                    AvtaleGjennomforing.StengtPeriode(
                        3,
    ...
    context("arena") {
        ...
        test("lagre arenatiltak") {
            database.runAndRollback {
                ...
                queries.gjennomforing.getArenaGjennomforingOrError(arenaEnkelAmo1.id).should {
                ...
                queries.gjennomforing.getArenaGjennomforing(arenaEnkelAmo1.id) shouldBe null
            }
        }
    }
    ...
    context("enkeltplass") {
        ...
        test("lagre enkeltplass") {
            database.runAndRollback {
                ...
                queries.gjennomforing.getEnkeltplassGjennomforingOrError(enkelAmo1.id).should {
                ...
                queries.gjennomforing.getEnkeltplassGjennomforing(enkelAmo1.id) shouldBe null
            }
        }
    }
    ...
    context("free text search") {
        test("løpenummer og tiltaksnummer blir automatisk med i fritekstsøket") {
            database.runAndRollback { session ->
                ...
                val lopenummer = queries.gjennomforing.getAvtaleGjennomforingOrError(Oppfolging1.id).lopenummer
    ...
    test("pagination") {
        database.runAndRollback { _ ->
            ...
            queries.gjennomforing.getAll(Pagination.all()).should {
                ...
                it.items.first().shouldBeTypeOf<AvtaleGjennomforingKompakt>().navn shouldBe "01"
                it.items.last().shouldBeTypeOf<AvtaleGjennomforingKompakt>().navn shouldBe "10"
            }
            ...
            queries.gjennomforing.getAll(Pagination.of(page = 1, size = 2)).should {
                it.totalCount shouldBe 10
                it.items.first().shouldBeTypeOf<AvtaleGjennomforingKompakt>().navn shouldBe "01"
                it.items.last().shouldBeTypeOf<AvtaleGjennomforingKompakt>().navn shouldBe "02"
            }

            queries.gjennomforing.getAll(Pagination.of(page = 3, size = 2)).should {
                it.totalCount shouldBe 10
                it.items.first().shouldBeTypeOf<AvtaleGjennomforingKompakt>().navn shouldBe "05"
                it.items.last().shouldBeTypeOf<AvtaleGjennomforingKompakt>().navn shouldBe "06"
            }

            queries.gjennomforing.getAll(Pagination.of(page = 3, size = 4)).should {
                it.totalCount shouldBe 10
                it.items.first().shouldBeTypeOf<AvtaleGjennomforingKompakt>().navn shouldBe "09"
                it.items.last().shouldBeTypeOf<AvtaleGjennomforingKompakt>().navn shouldBe "10"
            }
    ...
    context("Frikoble kontaktperson fra arrangør") {
        ...
        test("Skal fjerne kontaktperson fra koblingstabell") {
            database.runAndRollback { session ->
                ...
                queries.gjennomforing.getAvtaleGjennomforingOrError(domain.gjennomforinger[0].id).should {
                    it.arrangor.kontaktpersoner.first().id.shouldBe(kontaktperson1.id)
                }
                queries.gjennomforing.getAvtaleGjennomforingOrError(domain.gjennomforinger[1].id).should {
...
private fun toGjennomforingArrangorKontaktperson(kontaktperson: ArrangorKontaktperson) = AvtaleGjennomforing.ArrangorKontaktperson(
    id = kontaktperson.id,
