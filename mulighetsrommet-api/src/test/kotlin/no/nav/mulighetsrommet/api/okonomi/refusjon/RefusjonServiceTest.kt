...
class RefusjonServiceTest : FunSpec({
    ...
    context("Generering av refusjonskrav") {
        ...
        val tiltakshistorikk = mockk<TiltakshistorikkClient>()
        ...
        val service = RefusjonService(
            ...
            tiltakshistorikk = tiltakshistorikk,
            db = database.db,
        )

        test("generer et refusjonskrav") {
            tiltaksgjennomforingRepository.upsert(AFT1)
            coEvery { tiltakshistorikk.getDeltakelser(AFT1.id) } returns GruppetiltakDeltakelserResponse(deltakelser = emptyList())
