...
class TotrinnskontrollTest : FunSpec({
    ...
    context("opprett") {
        test("oppretter TIL_BEHANDLING med riktig behandletAv") {
            val opprettelse = Totrinnskontroll.opprett(UUID.randomUUID(), entityId, TotrinnskontrollType.TILSAGN_OPPRETTELSE, behandletAv)
        ...
        test("støtter årsaker og forklaring") {
            val opprettelse = Totrinnskontroll.opprett(UUID.randomUUID(), entityId, TotrinnskontrollType.TILSAGN_ANNULLERING, behandletAv, listOf("FEIL_PERIODE"), "Perioden er feil")
    ...
    context("godkjenn") {
        ...
        test("godkjenning uten årsaker-override beholder eksisterende årsaker") {
            val opprettelse = Totrinnskontroll.opprett(UUID.randomUUID(), entityId, TotrinnskontrollType.TILSAGN_ANNULLERING, behandletAv, listOf("FEIL_PERIODE"))
    ...
    context("settPaVent") {
        test("setter på vent") {
            val paVent = opprett(type = TotrinnskontrollType.ENKELTPLASS_OKONOMI)
    ...
    context("tilbakestill") {
        fun sattPaVent(): Totrinnskontroll = opprett(type = TotrinnskontrollType.ENKELTPLASS_OKONOMI)
            .settPaVent(besluttetAv, forklaring = "Trenger mer info").shouldBeRight()
        ...
        test("beholder eksisterende årsaker etter tilbakestilling") {
            val opprettelse = Totrinnskontroll.opprett(UUID.randomUUID(), entityId, TotrinnskontrollType.ENKELTPLASS_OKONOMI, behandletAv, listOf("FEIL_BELOP"))
            val paVent = opprettelse.settPaVent(besluttetAv, aarsaker = listOf("FEIL_BELOP"), forklaring = "Feil beløp").shouldBeRight()
        ...
        test("feiler når status er TIL_BEHANDLING") {
            opprett(type = TotrinnskontrollType.ENKELTPLASS_OKONOMI).tilbakestill(behandletAv) shouldBeLeft listOf(
                FieldError.of("Totrinnskontrollen kan bare tilbakestilles når den er satt på vent"),
            )
        }

        test("feiler når status er GODKJENT") {
            val godkjent = opprett(type = TotrinnskontrollType.ENKELTPLASS_OKONOMI).godkjenn(besluttetAv).shouldBeRight()
        ...
        test("feiler når status er RETURNERT") {
            val returnert = opprett(type = TotrinnskontrollType.ENKELTPLASS_OKONOMI).returner(besluttetAv).shouldBeRight()
