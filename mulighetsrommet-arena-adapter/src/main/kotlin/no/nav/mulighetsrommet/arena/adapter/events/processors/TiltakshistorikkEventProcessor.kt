...
class TiltakshistorikkEventProcessor(
    ...
    override suspend fun handleEvent(event: ArenaEvent): Either<ProcessingError, ProcessingResult> = either {
        ...
        val deltaker = TiltakshistorikkArenaDeltaker(
            ...
            arenaDeltakerId = event.arenaId.toInt(),
            arenaRegDato = ArenaUtils.parseTimestampAsInstant(data.REG_DATO),
            arenaModDato = ArenaUtils.parseTimestampAsInstant(data.MOD_DATO),
            norskIdent = NorskIdent(norskIdent),
            status = ArenaDeltakerStatus.valueOf(data.DELTAKERSTATUSKODE.name),
            startDato = ArenaUtils.parseNullableTimestamp(data.DATO_FRA)?.toLocalDate(),
            sluttDato = ArenaUtils.parseNullableTimestamp(data.DATO_TIL)?.toLocalDate(),
