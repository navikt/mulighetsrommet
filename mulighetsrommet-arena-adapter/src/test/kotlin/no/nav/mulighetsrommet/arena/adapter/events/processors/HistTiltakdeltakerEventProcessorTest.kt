...
class HistTiltakdeltakerEventProcessorTest : FunSpec({
    ...
    context("handleEvent") {
        ...
        fun createProcessor(engine: HttpClientEngine = createMockEngine()): HistTiltakdeltakerEventProcessor {
            ...
            return HistTiltakdeltakerEventProcessor(entities, client, ords)
        }
        ...
        context("when dependent events has not been processed") {
            test("should save the event with status Failed when the dependent tiltaksgjennomføring is missing") {
                ...
                val (event) = prepareEvent(createArenaHistTiltakdeltakerEvent(Insert))
        ...
        context("when dependent events has been processed") {
            ...
            beforeEach {
                ...
                listOf(tiltakstypeIndividuell).forEach {
                    ...
                    mappings.upsert(ArenaEntityMapping(ArenaTable.Tiltakstype, it.tiltakskode, it.id, Handled))
                }
            ...
            test("should upsert tiltakshistorikk when all services responds with success") {
                val (event, mapping) = prepareEvent(createArenaHistTiltakdeltakerEvent(Insert), Ignored)
                ...
                processor.handleEvent(createArenaHistTiltakdeltakerEvent(Delete)).shouldBeRight()
