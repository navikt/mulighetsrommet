...
class FakturaQueries(private val session: Session) {
    ...
    fun insertFaktura(faktura: Faktura) = withTransaction(session) {
        ...
        val params = mapOf(
            ...
            "status_sist_oppdatert" to faktura.statusSistOppdatert,
    ...
    fun getByFakturanummer(fakturanummer: Fakturanummer): Faktura? {
        ...
        return session.single(queryOf(selectFaktura, fakturanummer.value)) { row ->
            ...
            Faktura(
                ...
                statusSistOppdatert = row.instant("status_sist_oppdatert"),
                behandletAv = OkonomiPart.fromString(row.string("behandlet_av")),
                behandletTidspunkt = row.instant("behandlet_tidspunkt"),
                besluttetAv = OkonomiPart.fromString(row.string("besluttet_av")),
                besluttetTidspunkt = row.instant("besluttet_tidspunkt"),
                linjer = linjer,
