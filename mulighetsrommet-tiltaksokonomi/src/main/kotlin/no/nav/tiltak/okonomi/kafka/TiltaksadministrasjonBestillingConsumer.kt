...
class TiltaksadministrasjonBestillingConsumer(
    ...
    override suspend fun consume(key: String, message: JsonElement) {
        ...
        require(bestillingsnummer.value.startsWith("A-")) {
            "Ugyldig bestillingsnummer=$bestillingsnummer fra Tiltaksadministrasjon, må starte med 'A-'"
        }

        val melding = Json.decodeFromJsonElement<OkonomiBestillingMelding>(message)
        when {
            melding is OkonomiBestillingMelding.Bestilling -> {
                require(melding.payload.okonomiSystem == OkonomiSystem.TILTAKSADMINISTRASJON) {
                    "Ugyldig system ${melding.payload.okonomiSystem} fra Tiltaksadministrasjon"
                }
