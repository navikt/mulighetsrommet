...
class TiltaksadministrasjonBestillingConsumerTest : FunSpec({
    ...
    test("avviser melding når bestillingsnummer ikke starter med 'A-'") {
        ...
        exception.message shouldBe "Ugyldig bestillingsnummer=E-1-1 fra Tiltaksadministrasjon"
    ...
    test("delegerer gyldig bestilling til TiltaksokonomiService") {
        ...
        coEvery { okonomi.opprettBestilling(OkonomiFagsystem.TILTAKSADMINISTRASJON, melding.payload) } returns mockk<Bestilling>().right()

        consumer.consume("A-1-1", toJson(melding))

        coVerify(exactly = 1) { okonomi.opprettBestilling(OkonomiFagsystem.TILTAKSADMINISTRASJON, melding.payload) }
    }
})
