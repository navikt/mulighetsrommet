...
import no.nav.mulighetsrommet.kafka.toProducerRecordDbo
...
class OkonomiServiceTest : FunSpec({
    ...
    context("opprett bestilling") {
        ...
        test("skal opprette bestilling hos oebs og lagrer utgående melding om status for bestilling") {
            ...
            db.session { getLatestProducerRecord() }.should {
    ...
    context("frigjor bestilling") {
        ...
        test("frigjør bestilling = true setter siste fakturalinje i fakturaen til oebs og oppdaterer status på bestilling") {
            ...
            db.session { getLatestProducerRecord(topic = "faktura-status") }.should {
                it.value?.toString(Charsets.UTF_8) shouldBe Json.encodeToString(
                    FakturaStatus(
                        fakturanummer = "B2-F1",
                        status = FakturaStatusType.SENDT,
        ...
        test("frigjør faktura lager en faktura med erSisteLinje = true og setter bestillingen til FRIGJORT") {
            ...
            db.session { getLatestProducerRecord(topic = "bestilling-status") }.should {
                it.value?.toString(Charsets.UTF_8) shouldBe Json.encodeToString(
                    BestillingStatus(
...
private fun QueryContext.getLatestProducerRecord(topic: String? = null): ProducerRecordDbo {
    @Language("PostgreSQL")
    val sql = """
    ...
    return session.requireSingle(query) { it.toProducerRecordDbo() }
}
